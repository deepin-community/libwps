		...
		for (int b=0; b < numBytesByLines; ++b)
		{
			...
			for (int byt=0; byt<8; ++byt)
			{
				...
				for (int cBytes = 0, dpl=16; cBytes < 3; cBytes++, dpl-=8)
					data.append(static_cast<unsigned char>((col>>dpl)&0xFF));
			}
		}
	}
	return true;
...
void WPSGraphicStyle::addTo(librevenge::RVNGPropertyList &list, bool only1D) const
{
	...
	else if (m_lineDashWidth.size()>=2)
	{
		...
		const auto distance = double(nDots1 + nDots2 > 0 ? totalGap/float(nDots1+nDots2) : totalGap);
	...
	if (hasGradient())
	{
		...
		else
		{
			...
			for (auto s : m_gradientStopList)
			{
				...
				grad.insert("svg:stop-color", s.m_color.str().c_str());
				grad.insert("svg:stop-opacity", double(s.m_opacity), librevenge::RVNG_PERCENT);
...
void WPSGraphicStyle::addFrameTo(librevenge::RVNGPropertyList &list) const
{
	...
	if (!m_frameName.empty())
		list.insert("librevenge:frame-name",m_frameName);
}
...
std::ostream &operator<<(std::ostream &o, WPSGraphicStyle const &st)
{
	...
	{
		o << "dash=[";
		for (auto const &w : st.m_lineDashWidth)
			o << w << ",";
		o << "],";
	}
	switch (st.m_lineCap)
	...
	if (st.hasGradient())
	{
		...
		{
			o << "stops=[";
			for (auto const &stop : st.m_gradientStopList)
				o << "[" << stop << "],";
			o << "],";
		}
	...
	if (!st.m_frameName.empty())
		o << "frame[name]=" << st.m_frameName.cstr() << ",";
	if (!st.m_frameNextName.empty())
		o << "frame[linkedto]=" << st.m_frameNextName.cstr() << ",";
	o << st.m_extra;
	return o;
}
