...
WKSSubDocument::WKSSubDocument(RVNGInputStreamPtr const &input, WKSParser *p, int i)  : WPSSubDocument(input,i), m_parser(p)
{
}
...
bool WKSSubDocument::operator==(std::shared_ptr<WPSSubDocument> const &doc) const
{
	...
	auto const *docu=dynamic_cast<WKSSubDocument const *>(doc.get());
	if (docu==nullptr) return false;
